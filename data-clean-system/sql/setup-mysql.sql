...
CREATE TABLE `extra_data_title` (
                                    ...
                                    `extract_status` VARCHAR(20) DEFAULT NULL COMMENT '提取状态 RUNNING/SUCCESS/PARTIAL/FAILED',
                                    `extract_start_time` DATETIME DEFAULT NULL COMMENT '提取开始时间',
                                    `extract_end_time` DATETIME DEFAULT NULL COMMENT '提取结束时间',
                                    `extract_cost_ms` BIGINT DEFAULT NULL COMMENT '提取耗时(毫秒)',
                                    `row_count` INT DEFAULT NULL COMMENT '提取行数',
                                    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                    `created_by` VARCHAR(50) DEFAULT 'system' COMMENT '创建人',
                                    `updated_by` VARCHAR(50) DEFAULT 'system' COMMENT '更新人'

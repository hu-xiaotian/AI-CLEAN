 ...
 * AI 属性提取（跑批模式）提示词加载器。
 * <p>
 * 提示词独立存放在 {@code extract-batch-prompt.properties} 中，便于后续修改而无需改动代码。
 * 加载优先级：
 * <ol>
 *     <li>外部文件 {@code <外部提示词目录>/extract-batch-prompt.properties}（默认外部目录为
 *         启动目录下 {@code ./config}，可通过配置 {@code app.ai.extract-prompt-external-dir} 覆盖）——
 *         修改后保存即生效，适合现场调优。</li>
 *     <li>classpath 内置文件 {@code classpath:extract-batch-prompt.properties}（随包发布）。</li>
 * </ol>
 * 每次调用 {@link #get(String)} 都会重新检查外部文件，外部文件被修改后可立即读到新提示词。
 */
@Slf4j
@Component
public class BatchExtractPrompt {

    /** 内置提示词文件的 classpath 路径 */
    private static final String CLASS_PATH_FILE = "extract-batch-prompt.properties";

    /** 外部提示词目录（默认启动目录下 ./config） */
    @Value("${app.ai.extract-prompt-external-dir:./config}")
    ...
    private void loadClasspath() {
        try (InputStream in = new ClassPathResource(CLASS_PATH_FILE).getInputStream()) {
            ...
            log.info("属性提取提示词已加载（classpath），共 {} 条配置", classpathProps.size());
        } catch (IOException e) {
            log.warn("classpath 中未找到属性提取提示词文件 {}，将使用默认提示词", CLASS_PATH_FILE, e);
        }
    }
    ...
    private Properties loadExternal() {
        try {
            ...
            log.warn("读取外部属性提取提示词文件失败，回退内置: {}", e.getMessage());
            return null;
        }
    }
